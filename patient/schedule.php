 ...
 <div class="container">
     ...
     <table class="menu-container" border="0">
        ...
        <?php
                ...
                $sqlmain= "SELECT schedule.*, doctor.docname, doctor.specialties 
                          FROM schedule 
                          INNER JOIN doctor ON CAST(schedule.docid AS SIGNED) = doctor.docid 
                ...
                if($_POST){
                    if(!empty($_POST["search"])){
                        ...
                        $sqlmain = "SELECT schedule.*, doctor.docname, doctor.specialties, specialties.sname 
                                  FROM schedule 
                                  INNER JOIN doctor ON CAST(schedule.docid AS SIGNED) = doctor.docid 
                                  INNER JOIN specialties ON doctor.specialties = specialties.id
                                  WHERE schedule.scheduledate >= ? 
                                  AND specialties.sname LIKE ? 
                                  ORDER BY schedule.scheduledate ASC";
                        
                        $stmt = $database->prepare($sqlmain);
                        $search_term = "%" . $keyword . "%";
                        $stmt->bind_param("ss", $today, $search_term);
                        $stmt->execute();
                        $result = $stmt->get_result();
                ...
                <tr >
                    ...
                    <td >
                            <form action="" method="post" class="header-search">
                                        ...
                                        <input type="search" name="search" class="input-text header-searchbar" placeholder="Search by Specialization" list="specializations" value="<?php  echo $insertkey ?>">&nbsp;&nbsp;
                                        
                                        <?php
                                            echo '<datalist id="specializations">';
                                            $list11 = $database->query("select DISTINCT specialties from doctor ORDER BY specialties;");
